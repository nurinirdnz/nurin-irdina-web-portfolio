...
(async () => {
  ...
  const previews = {
    jomdekan: diagram(
      "Simplified request flow. Full architecture in the case study below.",
  ...
  $("#featured-grid").innerHTML = projects
    ...
    .map(
      (p, i) =>
        `<button class="project-card reveal" data-project="${p.id}" aria-label="View ${p.name} project details"><div class="project-visual${i === 0 ? "" : " compact"}"><div class="visual-meta"><span>0${i + 1} / ${p.category.toUpperCase()}</span><span>${i === 0 ? "FEATURED PROJECT" : "SELECTED WORK"}</span></div><div aria-hidden="true">${previews[p.id]}</div><div class="hover-tags">${tags(p.stack)}</div></div><div class="project-info"><div><h3>${p.name}</h3><p>${p.subtitle}</p></div><span class="round-arrow" aria-hidden="true">↗</span></div></button>`,
    )
    .join("");
  if (loadError)
    $("#featured-grid").innerHTML =
      '<p role="alert">Projects could not load. Please refresh the page to try again.</p>';
  function renderArchive(filter = "all") {
    $("#archive").innerHTML = projects
      .filter((p) => !p.featured && (filter === "all" || p.type === filter))
      .map(
        (p) =>
          `<button class="archive-item" data-project="${p.id}" aria-label="View ${p.name} project details"><span class="index">0${projects.indexOf(p) + 1}</span><div><h4>${p.name}</h4><p>${p.category}</p></div><span class="archive-stack">${p.stack.slice(0, 3).join(" / ")}</span><span class="round-arrow" aria-hidden="true">↗</span></button>`,
      )
      .join("");
  }
  renderArchive();
