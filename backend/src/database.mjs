...
export function openDatabase(path) {
  ...
  db.exec('BEGIN');
  try {
    seed.projects.forEach((p,i)=>db.prepare('INSERT OR IGNORE INTO projects VALUES (?, ?, ?)').run(p.id,i,JSON.stringify(p)));
    db.prepare('INSERT OR IGNORE INTO settings VALUES (?, ?)').run('email',seed.email);
    db.exec('COMMIT');
